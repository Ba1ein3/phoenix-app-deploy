...
export class IOOptionsDialogComponent implements OnInit {
  ...
  handleEDM4HEPJSONEventDataInput(files: FileList) {
    const callback = (content: any) => {
      ...
      const edm4hepJsonLoader = new Edm4hepJsonLoader({
        eventScale: this.eventDisplay.configuration.eventScale,
        magneticFieldStrength:
          this.eventDisplay.configuration.magneticFieldStrength,
      });
  ...
  handleGLTFInput(files: FileList) {
    ...
    this.handleFileInput(files[0], 'gltf,glb,zip', callback);
  }
  ...
  private isFileOfExtension(fileName: string, extensions: string): boolean {
    ...
    // use lastIndexOf to find the suffix when the filename contains multiple .
    const dotIndex = fileName.lastIndexOf('.');
    if (dotIndex === -1) {
      console.error('Error: No file extension found!');
      this.eventDisplay
        .getInfoLogger()
        .add('No file extension found!', 'Error');
      return false;
    }
  ...
  exportGeometries() {
    this.eventDisplay.exportGeometriesToGLTF();
  }
}
