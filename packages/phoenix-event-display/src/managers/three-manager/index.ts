...
export class ThreeManager {
  ...
  private verticalClipPlanes: Plane[];
  private horizontalClipPlanes: Plane[];
  private clipPlanes: Plane[];
  /** The type of clipping **/
  private clipType: 'vertical' | 'horizontal';
  ...
  public init(configuration: Configuration) {
    // Set the clipping planes. The vertical mode clips geometry along the x-y plane
    this.horizontalClipPlanes = [
      // these 2 planes are used internally for the clipping functionnality
      new Plane(new Vector3(0, 1, 0), 0),
      new Plane(new Vector3(0, -1, 0), 0),
    ];
    this.verticalClipPlanes = [new Plane(new Vector3(0, 0, -1), 0)];
    this.clipPlanes = [];
    // Scene manager
  ...
  public setClipType(type: 'horizontal' | 'vertical') {
    if (!(this.clipType == type)) {
      this.clipType = type;
      if (type == 'horizontal') {
        this.clipPlanes.length = 0;
        this.clipPlanes.push(...this.horizontalClipPlanes);
      }
      if (type == 'vertical') {
        this.clipPlanes.length = 0;
        this.clipPlanes.push(...this.verticalClipPlanes);
      }
    }
  }
   ...
   * @param clippingEnabled If the geometry clipping is to be enabled or disabled.
   */
  ...
  public setClippingAngle(startingAngle: number, openingAngle: number) {
    ...
    this.horizontalClipPlanes[0].normal
      .set(0, -1, 0)
    ...
    this.horizontalClipPlanes[1].normal
      .set(0, 1, 0)
  ...
  /**
   * Exports geometry as gltf format, allowing to
   * load it later and recover the saved geometry.
   */
  public exportGeometriesToGLTF() {
    this.exportManager.exportGeometriesToGLTF(
