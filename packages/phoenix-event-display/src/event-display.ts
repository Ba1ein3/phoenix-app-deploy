...
export class EventDisplay {
  ...
  /**
   * Exports current geometry as gltf format
   */
  public exportGeometriesToGLTF() {
    this.graphicsLibrary.exportGeometriesToGLTF();
  }
  ...
  public async loadRootGeometry(
    ...
    // Set numfaces and numnodes
    let opt: { dflt_colors: boolean; numfaces?: number; numnodes?: number } = {
      dflt_colors: true,
    };
    if (this.configuration.maxNumFaces & this.configuration.maxNumNodes) {
      opt = {
        dflt_colors: true,
        numfaces: this.configuration.maxNumFaces,
        numnodes: this.configuration.maxNumNodes,
      };
      console.log('Setting the numfaces and numnodes to:', opt);
    }
    await this.loadJSONGeometry(
      build(obj, opt).toJSON(),
      name,
      menuNodeName,
      scale,
      doubleSided,
