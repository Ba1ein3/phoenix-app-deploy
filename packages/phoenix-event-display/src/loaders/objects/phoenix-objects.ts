...
import { TrackInterpolation } from '../../helpers/runge-kutta';
...
export class PhoenixObjects {
  ...
  public static getTrack(trackParams: any): Object3D {
    ...
    const size = trackParams.size ?? 2;
    // attributes
    const curve = new CatmullRomCurve3(points);

    // TubeGeometry
    const geometry = new TubeGeometry(curve, undefined, size);
  ...
  public static getCluster(
    ...
    const clusterLength = clusterParams.length;
  ...
  public static getMCParticle(MCParticleParams: any): Object3D {
    ...
    if (MCParticleParams['Vertex']) {
      vertex.x = MCParticleParams.Vertex.x;
      vertex.y = MCParticleParams.Vertex.y;
      vertex.z = MCParticleParams.Vertex.z;
    }
    points.push(vertex);

    const momentum = new Vector3(0, 0, 0);
    if (MCParticleParams['Momentum']) {
      momentum.x = MCParticleParams.Momentum.Px;
      momentum.y = MCParticleParams.Momentum.Py;
      momentum.z = MCParticleParams.Momentum.Pz;
    }
    const size = MCParticleParams['size'];

    const endpoint = new Vector3(0, 0, 0);
    if (MCParticleParams['Endpoint']) {
      endpoint.x = MCParticleParams.Endpoint.x;
      endpoint.y = MCParticleParams.Endpoint.y;
      endpoint.z = MCParticleParams.Endpoint.z;
    } else {
      endpoint.x = momentum.clone().normalize().x * 50 * length;
      endpoint.y = momentum.clone().normalize().y * 50 * length;
      endpoint.z = momentum.clone().normalize().z * 50 * length;
    }
    points.push(endpoint);
    const magneticFieldStrength = MCParticleParams.magneticFieldStrength;
    let track;
    if (MCParticleParams['Charge'] == 0) {
      // If the MC particle is neutral, always happen in cases of neutrinos
      const geometry = new BufferGeometry().setFromPoints(points);

      // material
      if (
        Math.abs(MCParticleParams['PDG']) == 14 ||
        Math.abs(MCParticleParams['PDG']) == 16 ||
        Math.abs(MCParticleParams['PDG']) == 18
      ) {
        // If the MC particles are neutrinos
        const material = new LineDashedMaterial({
          linewidth: size,
          dashSize: 2,
          color: MCParticleParams.color ?? new Color(0x007799),
          transparent: true,
          opacity: 0.5,
        });
        // MC particle's track
        track = new Line(geometry, material);
        ...
        track = new LineSegments(geometry, material);
      }
    } else {
      const curve_points = TrackInterpolation.interpolate(
        points[0],
        points[1],
        momentum,
        64,
      ...
      const tubeGeometry = new TubeGeometry(curve, 128, size, 8, false);
      ...
      track = new Mesh(tubeGeometry, material);
    }

    track.userData = Object.assign({}, MCParticleParams);
    track.name = 'MCParticle';
    const object = new Group();
    if (!(endpoint.x == 0 && endpoint.y == 0 && endpoint.z == 0)) {
      const sphereGeometry = new SphereGeometry(size * 2);
      const material = new MeshBasicMaterial({
        color: MCParticleParams.color ?? new Color(0x007799),
      });
      const particleBall = new Mesh(sphereGeometry, material);

      particleBall.position.set(endpoint.x, endpoint.y, endpoint.z);
      particleBall.userData = Object.assign({}, MCParticleParams);
      particleBall.name = 'MCParticle';
      object.add(particleBall);
    }
    object.add(track);
    MCParticleParams.uuid = object.uuid;
